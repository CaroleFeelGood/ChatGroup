...
//   //   }
//   let sessionId;
//   if (request.headers.cookie) sessionId = request.headers.cookie.split('=')[1];
...
//   let memberConnected = getMember(sessions[sessionId]);
//   if (!memberConnected) {
//     console.log('False sessions[sessionId]', sessions[sessionId]);
//     socket.destroy();
//     return;
//   }
//   console.log('true sessions[sessionId]', sessions[sessionId]);
...
websocketServer.on('connection', function connection(webSocketClient, request, clients) {
  ...
  webSocketClient.on('message', message => {
    ...
    // console.log(`Received message ${message} from user ${request}`);
